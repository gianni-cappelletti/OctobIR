...
Convolution::Convolution()
    : impulseLength(0), bufferSize(0), writeIndex(0), bufferMask(0), isPowerOfTwo(false) {}
...
float Convolution::process(const float inputSample) {
    ...
    writeIndex = (isPowerOfTwo) ? ((writeIndex + 1) & bufferMask) : ((writeIndex + 1) % bufferSize);
...
int Convolution::getImpulseLength() const { return impulseLength; }
